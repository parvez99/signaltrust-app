...
export function signalCareerVelocity(profile: any) {
    ...
    for (const r of sorted) {
      ...
      if (isDirectorPlus(title) && years < 4) {
        bestHit = { kind: "director_plus", role: r, years };
      } else if (isSenior(title) && years < 2 && !bestHit) {
        bestHit = { kind: "senior", role: r, years };
      }
    }
    ...
    const deduction = bestHit.kind === "director_plus" ? 12 : 10;
    ...
    return makeSignal({
      ...
      status: "triggered",
      evidence: {
        years_from_first_role: Number(bestHit.years.toFixed(2)),
